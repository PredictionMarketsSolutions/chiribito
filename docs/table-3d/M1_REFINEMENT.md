...
- **Status:** ✅ desktop M1 refinement COMPLETE. TP0 is ready for its baseline freeze (plan 01-06) — **awaiting the operator's explicit go** (the freeze persists the apples-to-apples baseline anchor; local + reversible, no push). Baseline still UNFROZEN until then.
